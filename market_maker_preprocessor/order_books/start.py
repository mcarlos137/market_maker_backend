...
from order_books.base import OrderBooksPeriodDataType
from damexCommons.tools.dbs import get_exchange_db

    
def run_order_books_period_data_type_thread(exchange: str, base_asset: str, quote_asset: str):
    order_books_period_data_type = OrderBooksPeriodDataType(
        exchange=exchange, 
        base_asset=base_asset,
        quote_asset=quote_asset
    )
    order_books_period_data_type.run()
...
for exchange in exchanges_info:
    for market in exchanges_info[exchange]:
        ...
        if exchanges_info[exchange][market]['preprocess']:
            t = Thread(target=run_order_books_period_data_type_thread, args=(exchange, base_asset, quote_asset))
            t.start()    
