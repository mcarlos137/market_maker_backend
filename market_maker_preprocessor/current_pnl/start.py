...
from current_pnl.base import CurrentPnlPeriodDataType
from damexCommons.tools.dbs import get_exchange_db

    
def run_current_pnl_period_data_type_thread(exchange: str, base_asset: str, quote_asset: str):
    current_pnl_period_data_type = CurrentPnlPeriodDataType(
        exchange=exchange, 
        base_asset=base_asset,
        quote_asset=quote_asset
    )
    current_pnl_period_data_type.run()
...
for exchange in exchanges_info:
    for market in exchanges_info[exchange]:
        ...
        if exchanges_info[exchange][market]['preprocess']:
            t = Thread(target=run_current_pnl_period_data_type_thread, args=(exchange, base_asset, quote_asset))
            t.start()    
