...
class ArbitrageOpportunities:
    ...
    def __init__(self, arbitrage_type: str) -> None:
        ...
        self.operation = 'arbitrage_opportunities'
        self.min_profitability_percentage = 0.01
        self.order_books = {}
        self.arbitrage_opportunities = {}
        self.last_arbitrage_opportunities = {}
        
        self.queue_receive_connection = pika.BlockingConnection(
        ...
        self.queue_send_connection = pika.BlockingConnection(
            pika.ConnectionParameters('172.20.143.247',
                5672,
                '/',
                pika.PlainCredentials('test', 'test')
        ...
        self.order_book_channel = self.queue_receive_connection.channel()
        self.arbitrage_opportunity_simple_channel = self.queue_send_connection.channel()
        self.arbitrage_opportunity_simple_channel.queue_declare(queue='arbitrage_opportunity.simple', arguments={'x-max-length': 1})
        self.arbitrage_opportunity_simple_channel.queue_bind(queue='arbitrage_opportunity.simple', exchange='damex', routing_key='arbitrage_opportunity.simple')        
    ...
    def run(self):
        ...
        t_1 = Thread(target=self.check_arbitrage_opportunities_1, args=())
        t_1.start()
        t_2 = Thread(target=self.check_arbitrage_opportunities_2, args=())
        t_2.start()
        while True:
            try:
                self.order_book_channel.basic_consume(queue='order_book',
                    auto_ack=True,
                    on_message_callback=self.callback
                )
                self.order_book_channel.start_consuming()
    ...
    def get_current_arbitrage_opportunities(self):
        info_file = '/home/ubuntu/arbitrage_opportunities/' + self.arbitrage_type + '/' + 'info.json'
        if os.path.isfile(info_file):
            info = json.loads(open(info_file, 'r', encoding='UTF-8').read())
            if 'last_file_name' in info:
                last_file = '/home/ubuntu/arbitrage_opportunities/' + self.arbitrage_type + '/' + info['last_file_name']
                if os.path.isfile(last_file):
                    return json.loads(open(last_file, 'r', encoding='UTF-8').read())
        return {}

    def update_arbitrage_opportunities_file(self):
    ...
    def send_arbitrage_opportunities_to_queue(self):
        print('SEND TO QUEUE')
        self.arbitrage_opportunity_simple_channel.basic_publish(exchange='damex',
            routing_key='arbitrage_opportunity.simple',
            body=json.dumps(self.arbitrage_opportunities)
        )
            
    def check_arbitrage_opportunities_1(self):
        while True:
            logging.info('----------------- start check_arbitrage_opportunities_1 -----------------')
            logging.info('------------------------ %s', len(self.arbitrage_opportunities))
            update = False
            for key in list(self.arbitrage_opportunities):
                if self.arbitrage_opportunities[key]['last_time'] < int(time.time() * 1e3) - (30 * 1e3):
                    update = True
                    del self.arbitrage_opportunities[key]
            
            if update:
                self.update_arbitrage_opportunities_file()
            logging.info('----------------- end check_arbitrage_opportunities_1 -----------------')
            time.sleep(10)
            
    def check_arbitrage_opportunities_2(self):
        time.sleep(5)
        while True:
            logging.info('----------------- start check_arbitrage_opportunities_2 -----------------')
            logging.info('------------------------ %s %s', len(self.arbitrage_opportunities), len(self.last_arbitrage_opportunities))
            if self.last_arbitrage_opportunities != self.arbitrage_opportunities:
                self.last_arbitrage_opportunities = dict(self.arbitrage_opportunities)
                self.send_arbitrage_opportunities_to_queue()
            logging.info('----------------- end check_arbitrage_opportunities_2 -----------------')
            time.sleep(1)
            ...
            for exchange_combination in exchange_combinations:
                ...
                if key not in self.arbitrage_opportunities:
                    self.arbitrage_opportunities[key] = {
                        ...
                        'buy_price': first_ask_price,
                        'sell_price': first_bid_price
                    }
                    self.update_arbitrage_opportunities_file()
                ...
                self.arbitrage_opportunities[key]['buy_price'] = first_ask_price   
                self.arbitrage_opportunities[key]['sell_price'] = first_bid_price
