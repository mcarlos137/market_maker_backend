...
from ..watchers_wss.current_orders import WatcherWssCurrentOrders
from .base import ConsumerBase

WSS_ROUTE = 'current_orders'
ALLOWED_OPERATIONS = ['get_count', 'get_amounts', 'get_mid_prices']

watcher_wss_current_orders = None

class ConsumerCurrentOrders(ConsumerBase):
    
    def __init__(self, *args, **kwargs):
        kwargs['wss_route'] = WSS_ROUTE
        kwargs['allowed_operations'] = ALLOWED_OPERATIONS
        global watcher_wss_current_orders
        if watcher_wss_current_orders is None:
            watcher_wss_current_orders = WatcherWssCurrentOrders()
            watcher_wss_current_orders.run()
