...
from ..watchers_wss.current_values import WatcherWssCurrentValues
from .base import ConsumerBase

WSS_ROUTE = 'current_values'
ALLOWED_OPERATIONS = ['get']

watcher_wss_current_values = None

class ConsumerCurrentValues(ConsumerBase):
    
    def __init__(self, *args, **kwargs):
        kwargs['wss_route'] = WSS_ROUTE
        kwargs['allowed_operations'] = ALLOWED_OPERATIONS
        global watcher_wss_current_values
        if watcher_wss_current_values is None:
            watcher_wss_current_values = WatcherWssCurrentValues()
            watcher_wss_current_values.run()
    ...
    def add_old_data_callback(self, *args, **kwargs):
        ...
        final_old_data = []
        data_folder = '%s/%s/%s/%s' % (DATA_FOLDER, exchange,ROUTE_OPERATIONS[WSS_ROUTE][operation]['folders'][0], 'NONE')
        data_folder_old = '%s/%s/%s/%s' % (DATA_OLD_FOLDER, exchange, ROUTE_OPERATIONS[WSS_ROUTE][operation]['folders'][0], 'NONE')
